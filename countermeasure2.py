...
def bus_off_attack_with_countermeasure2():
    ...
    def attack_cycle(victim: ECU, attacker: ECU):
        ...
        if phase == "PHASE1":
            ...
            if victim.TEC >= 256:
                print(f"\n[END] {victim.name} is BUS-OFF! Attack completed.")
                return  # end of attack
            if attacker.TEC >= 256:
                print(f"\n[END] {attacker.name} is BUS-OFF! Defense successful.")
                return  # end of attack
        ...
        # next cycle
        threading.Timer(0.5, partial(attack_cycle, victim, attacker)).start()
    ...
    threading.Timer(0.1, partial(attack_cycle(victim=attacker, attacker=guardian_ecu))).start()
    threading.Timer(0.1, partial(attack_cycle(victim=victim, attacker=attacker))).start()